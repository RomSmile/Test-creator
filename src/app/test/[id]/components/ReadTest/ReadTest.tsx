...
const ReadTest: FC<IReadTest> = ({ id }) => {
  ...
  const [ currentTest ] = useState<ITest>(tests.find(item => item.testId === id));
  const [ isTestFinished, setIsTestFinished ] = useState<boolean>(false);

  const [validateArray, setValidateArray]
    = useState<IValidateArrayItem[]>(currentTest.questions.map((question) => {
      return {
  ...
  const selectAnswer = (questionIndex: number, answerIndex: number) => {
    ...
    setValidateArray((validateArray) => {
      ...
      newValidateArray[questionIndex].answers = validateArray[questionIndex].answers.map((item, index) => {
  ...
  const finishTest = () => {
    ...
    const newValidateArray = [...validateArray].map((question, questionIndex) => {
      const newQuestion = {...question}
      newQuestion.answers = newQuestion.answers.map((answer, answerIndex) => {
        if (currentTest.questions[questionIndex].answers[answerIndex].isCorrect && answer.isSelected) {
  ...
  return (
    <div className={s.questionsContainer}>
      ...
      {isTestFinished ? (
        <Link href="/" className={s.testButtons}>Back to list</Link>
      ) : (
        <button
